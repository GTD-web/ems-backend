...
/**
 * 프로젝트 2차 평가자 설정 API 데코레이터
 */
export function SetSecondaryEvaluators() {
  return applyDecorators(
    Post(':id/secondary-evaluators'),
    HttpCode(HttpStatus.OK),
    ApiOperation({
...
- 잘못된 UUID 형식: UUID 형식이 아닌 평가자 ID 입력 시 400 에러`,
    ...
    ApiBody({ type: SetSecondaryEvaluatorsDto }),
    ApiResponse({
      status: HttpStatus.OK,
      description: '2차 평가자가 성공적으로 설정되었습니다.',
      type: SetSecondaryEvaluatorsResponseDto,
    }),
    ApiResponse({
      status: HttpStatus.NOT_FOUND,
      description: '프로젝트를 찾을 수 없습니다.',
