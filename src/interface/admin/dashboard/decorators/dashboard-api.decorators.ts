...
export function GetMyEvaluationTargetsStatus() {
  return applyDecorators(
    ...
    ApiOperation({
      ...
      description: `평가자가 자신이 담당하는 피평가자들의 평가 현황을 배열로 조회합니다.
...
**테스트 케이스:**
- 평가자가 담당하는 평가 대상자 현황을 조회할 수 있어야 한다
- PRIMARY와 SECONDARY 평가자 구분이 정확해야 한다
- 제외된 피평가자도 조회되어야 한다 (제외 여부 정보 포함)
- 응답에 모든 필수 필드가 포함되어야 한다 (제외정보, 평가항목, WBS기준, 평가라인, 성과입력, 하향평가)
- 담당하는 평가 대상자가 없으면 빈 배열을 반환해야 한다
- 여러 피평가자를 담당하는 경우 모두 조회되어야 한다
- 성과 입력 상태가 정확해야 한다
- 존재하지 않는 평가기간 조회 시 빈 배열을 반환해야 한다
- 존재하지 않는 평가자 조회 시 빈 배열을 반환해야 한다
- 잘못된 평가기간 UUID 형식으로 요청 시 에러가 발생해야 한다
- 잘못된 평가자 UUID 형식으로 요청 시 에러가 발생해야 한다
- 여러 피평가자의 데이터가 섞이지 않아야 한다
- 상태 값이 예상된 enum 값 중 하나여야 한다`,
    }),
    ApiParam({
      name: 'evaluationPeriodId',
