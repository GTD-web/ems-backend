...
describe('PATCH /admin/evaluation-periods/:id/settings/final-evaluation-permission (E2E)', () => {
  ...
  describe('성공 케이스', () => {
    it('최종 평가 설정 수동 허용을 true로 변경 시 200 응답을 반환해야 한다', async () => {
      // Given
      const updateData = {
  ...
  describe('비즈니스 로직 테스트', () => {
    it('최종 평가 설정과 평가 기준 설정은 독립적으로 동작해야 한다', async () => {
      // Given - 평가 기준 설정을 먼저 변경
      await request(app.getHttpServer())
        .patch(
          `/admin/evaluation-periods/${evaluationPeriodId}/settings/criteria-permission`,
        )
        .send({ allowManualSetting: false })
        .expect(200);
      ...
      // Then - 각각 독립적으로 설정되어야 함
      expect(response.body.criteriaSettingEnabled).toBe(false);
      expect(response.body.finalEvaluationSettingEnabled).toBe(true);
    });

    it('최종 평가 설정 변경이 자기 평가 설정에 영향을 주지 않아야 한다', async () => {
      // Given - 자기 평가 설정을 먼저 변경
      await request(app.getHttpServer())
        .patch(
      ...
      // Then - 자기 평가 설정은 변경되지 않아야 함
      expect(response.body.selfEvaluationSettingEnabled).toBe(false);
      expect(response.body.finalEvaluationSettingEnabled).toBe(true);
    });

    it('모든 설정을 개별적으로 변경해도 서로 영향을 주지 않아야 한다', async () => {
      // Given - 모든 설정을 false로 초기화
      await request(app.getHttpServer())
        .patch(
          `/admin/evaluation-periods/${evaluationPeriodId}/settings/criteria-permission`,
        )
