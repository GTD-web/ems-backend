/**
 * 평가 설정 마감일 수정 - 실제 데이터 기반 E2E 테스트
 */
...
describe('PATCH /admin/evaluation-periods/:id/evaluation-setup-deadline - 실제 데이터 기반', () => {
  ...
  describe('시나리오 1: 성공 케이스', () => {
    it('평가 설정 마감일을 수정해야 한다', async () => {
      ...
      if (periods.length > 0) {
        ...
        const updateData = {
          evaluationSetupDeadline: newDeadline.toISOString().split('T')[0],
        };
        ...
        expect(response.body.evaluationSetupDeadline).toBeDefined();
