...
describe('PATCH /admin/evaluation-periods/:id/self-evaluation-deadline - 실제 데이터 기반', () => {
  ...
  describe('시나리오 1: 성공 케이스', () => {
    it('자기평가 마감일을 수정해야 한다', async () => {
      ...
      if (periods.length > 0) {
        ...
        // 마감일 순서: performanceDeadline < selfEvaluationDeadline < peerEvaluationDeadline
        const performanceDeadline = period.performanceDeadline
          ? new Date(period.performanceDeadline)
          : new Date(period.startDate);

        const peerEvaluationDeadline = new Date(period.peerEvaluationDeadline);
        ...
        const updateData = {
          selfEvaluationDeadline: newDeadline.toISOString().split('T')[0],
        };
