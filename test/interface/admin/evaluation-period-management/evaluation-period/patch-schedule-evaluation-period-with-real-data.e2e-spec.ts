...
describe('PATCH /admin/evaluation-periods/:id/schedule - 실제 데이터 기반', () => {
  ...
  describe('시나리오 1: 성공 케이스', () => {
    it('평가기간 스케줄을 수정해야 한다', async () => {
      ...
      if (periods.length > 0) {
        ...
        // 각 마감일을 시작일 기준으로 설정
        const performanceDeadline = new Date(startDate);
        performanceDeadline.setMonth(performanceDeadline.getMonth() + 2);

        const selfEvaluationDeadline = new Date(performanceDeadline);
        selfEvaluationDeadline.setMonth(selfEvaluationDeadline.getMonth() + 1);

        const peerEvaluationDeadline = new Date(selfEvaluationDeadline);
        peerEvaluationDeadline.setMonth(peerEvaluationDeadline.getMonth() + 1);

        const updateData = {
          performanceDeadline: performanceDeadline.toISOString().split('T')[0],
        ...
        expect(response.body.selfEvaluationDeadline).toBeDefined();
  ...
  describe('시나리오 2: 완료된 평가기간', () => {
    it('완료된 평가 기간의 스케줄 수정은 실패해야 한다', async () => {
      const periods = await dataSource
        .getRepository('EvaluationPeriod')
        .createQueryBuilder('period')
        .where('period.deletedAt IS NULL')
      ...
      if (periods.length > 0) {
        ...
        const updateData = {
          ...
          selfEvaluationDeadline: '2024-08-31',
