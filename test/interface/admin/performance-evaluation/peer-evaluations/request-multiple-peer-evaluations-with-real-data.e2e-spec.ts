/**
 * 동료평가 일괄 요청 (1:N) - 실제 데이터 기반 E2E 테스트
 *
 * 원본 테스트 케이스 26개를 모두 시드 데이터 기반으로 마이그레이션
 */
...
describe('POST /admin/performance-evaluation/peer-evaluations/requests/bulk/one-evaluator-to-many-evaluatees (실제 데이터)', () => {
  ...
  async function getThreeEvaluatees() {
    const result = await dataSource.query(
  ...
  async function bulkRequestPeerEvaluations(data: any) {
    return testSuite
      .request()
      .post(
  ...
  describe('동료평가 일괄 요청 성공 시나리오', () => {
    it('기본 일괄 요청을 생성할 수 있어야 한다', async () => {
      ...
      const evaluatees = await getThreeEvaluatees();
      const period = await getEvaluationPeriod();
      if (!evaluator || !evaluatees || !period) {
        console.log('데이터가 없어서 테스트 스킵');
        return;
      }

      const response = await bulkRequestPeerEvaluations({
    ...
    it('질문 ID 목록을 포함하여 일괄 요청을 생성할 수 있어야 한다', async () => {
      ...
      if (!evaluator || !evaluatees || !period || !question) {
        console.log('데이터가 없어서 테스트 스킵');
        return;
      }
    ...
    it('단일 피평가자에게도 요청할 수 있어야 한다', async () => {
      const evaluator = await getEvaluator();
      const evaluatees = await getThreeEvaluatees();
      const period = await getEvaluationPeriod();
      if (!evaluator || !evaluatees || !period) {
  ...
  describe('동료평가 일괄 요청 실패 시나리오', () => {
    ...
    it('evaluateeIds 생략 시 400 에러가 발생해야 한다', async () => {
      const evaluator = await getEvaluator();
      const period = await getEvaluationPeriod();
      if (!evaluator || !period) {
        console.log('데이터가 없어서 테스트 스킵');
    ...
    it('존재하지 않는 evaluateeId 포함 시 해당 피평가자는 건너뛰고 나머지를 생성해야 한다', async () => {
      const evaluator = await getEvaluator();
      const evaluatees = await getThreeEvaluatees();
      const period = await getEvaluationPeriod();
      if (!evaluator || !evaluatees || !period) {
      ...
      const response = await bulkRequestPeerEvaluations({
        evaluatorId: evaluator.id,
        evaluateeIds: [...evaluatees, uuidv4()],
        periodId: period.id,
      });
  ...
  describe('동료평가 일괄 요청 데이터 무결성 검증', () => {
      ...
      if (response.status === HttpStatus.CREATED && response.body.ids) {
        for (const id of response.body.ids) {
          ...
          if (dbRecord.length > 0) {
            expect(dbRecord[0].evaluatorId).toBe(evaluator.id);
            expect(evaluatees).toContain(dbRecord[0].evaluateeId);
    ...
    it('피평가자 목록에 평가자 본신을 포함할 경우 제외되어야 한다', async () => {
      const evaluator = await getEvaluator();
      const evaluatees = await getThreeEvaluatees();
      const period = await getEvaluationPeriod();
      if (!evaluator || !evaluatees || !period) {
      ...
      const response = await bulkRequestPeerEvaluations({
        evaluatorId: evaluator.id,
        evaluateeIds: [evaluator.id, ...evaluatees],
        periodId: period.id,
      });
