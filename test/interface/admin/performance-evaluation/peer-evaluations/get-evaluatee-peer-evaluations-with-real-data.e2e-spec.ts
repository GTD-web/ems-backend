/**
 * 피평가자의 동료평가 목록 조회 - 실제 데이터 기반 E2E 테스트
 */
...
describe('GET /admin/performance-evaluation/peer-evaluations/evaluatee/:evaluateeId (실제 데이터)', () => {
  ...
  async function getEvaluateeWithPeerEvaluations() {
  ...
  async function getEvaluationWithAnswersAndScore() {
    const result = await dataSource.query(
      `SELECT pe.id, pe."evaluateeId", pem."questionId"
       FROM peer_evaluation pe
       INNER JOIN peer_evaluation_question_mapping pem 
         ON pem."peerEvaluationId" = pe.id 
         AND pem."deletedAt" IS NULL
  ...
  describe('성공 케이스', () => {
    it('기본 목록: 동료평가 목록을 조회할 수 있어야 한다', async () => {
      const evaluateeId = await getEvaluateeWithPeerEvaluations();
      if (!evaluateeId) {
      ...
      const response = await testSuite
        .request()
        .get(
          `/admin/performance-evaluation/peer-evaluations/evaluatee/${evaluateeId}`,
        )
    ...
    it('필터링: evaluatorId로 필터링할 수 있어야 한다', async () => {
      const evaluateeId = await getEvaluateeWithPeerEvaluations();
    ...
    it('데이터 타입 검증: 모든 필드의 타입이 올바른지 확인한다', async () => {
      ...
      if (evaluationWithScore) {
        evaluateeId = evaluationWithScore.evaluateeId;
    ...
    it('evaluatorId 필터링: 특정 평가자만 필터링할 수 있어야 한다', async () => {
      const evaluateeId = await getEvaluateeWithPeerEvaluations();
      const evaluatorId = await getEvaluatorId();
      ...
      console.log('\n✅ evaluatorId 필터링 검증 완료');
    });

    it('periodId 필터링: 특정 평가기간만 필터링할 수 있어야 한다', async () => {
      const evaluateeId = await getEvaluateeWithPeerEvaluations();
      const periodId = await getPeriodId();
    ...
    it('복합 필터링: evaluatorId와 periodId를 함께 사용할 수 있어야 한다', async () => {
      const evaluateeId = await getEvaluateeWithPeerEvaluations();
      const evaluatorId = await getEvaluatorId();
      const periodId = await getPeriodId();

      if (!evaluateeId || !evaluatorId || !periodId) {
        console.log('데이터가 없어서 테스트 스킵');
        return;
      }
  ...
  describe('실패 케이스', () => {
    it('존재하지 않는 피평가자 ID로 조회 시 빈 목록을 반환해야 한다', async () => {
      const nonExistentId = '00000000-0000-0000-0000-000000000000';

      const response = await testSuite
        .request()
        .get(
          `/admin/performance-evaluation/peer-evaluations/evaluatee/${nonExistentId}`,
        )
        .expect(HttpStatus.OK);
    ...
    it('잘못된 UUID 형식으로 요청 시 400 에러가 발생해야 한다', async () => {
      ...
      await testSuite
        .request()
        .get(
          `/admin/performance-evaluation/peer-evaluations/evaluatee/${invalidId}`,
        )
