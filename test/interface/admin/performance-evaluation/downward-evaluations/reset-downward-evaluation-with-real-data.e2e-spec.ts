/**
 * 하향평가 초기화(미제출 상태 변경) - 실제 데이터 기반 E2E 테스트
 *
 * full 시나리오의 완료된 하향평가를 미제출 상태로 되돌리는 테스트입니다.
 */
...
describe('POST /admin/performance-evaluation/downward-evaluations - 초기화 (실제 데이터)', () => {
  ...
  async function getCompletedPrimaryEvaluation() {
  ...
  async function getCompletedSecondaryEvaluation() {
    const evaluations = await dataSource.query(
      `SELECT de.id, de."employeeId", de."evaluatorId", de."periodId", de."wbsId"
       FROM downward_evaluation de
  ...
  describe('1차 하향평가 초기화', () => {
    it('완료된 1차 하향평가를 미제출 상태로 변경할 수 있어야 한다', async () => {
      ...
      // Then - 응답이 비어있을 수 있으므로 DB에서 직접 확인
      ...
      // isCompleted가 false로 변경되었는지 확인
      expect(updatedEval[0].isCompleted).toBe(false);

      console.log('\n✅ 1차 하향평가 초기화 성공');
    });

    it('미완료 1차 하향평가는 초기화할 수 없어야 한다', async () => {
      const evaluations = await dataSource.query(
        `SELECT de.id, de."employeeId", de."evaluatorId", de."periodId", de."wbsId"
         FROM downward_evaluation de
         WHERE de."evaluationType" = 'primary'
  ...
  describe('2차 하향평가 초기화', () => {
    it('완료된 2차 하향평가를 미제출 상태로 변경할 수 있어야 한다', async () => {
  ...
  describe('실패 시나리오', () => {
    it('존재하지 않는 평가를 초기화할 수 없어야 한다', async () => {
      // 평가라인 매핑이 있는 조합 찾기
      ...
      // When - 존재하지 않는 평가 초기화 시도
      await testSuite
        .request()
        .post(
          `/admin/performance-evaluation/downward-evaluations/evaluatee/${mapping.employeeId}/period/${mapping.periodId}/wbs/${mapping.wbsItemId}/primary/reset`,
        )
        .send({ evaluatorId: mapping.evaluatorId })
        .expect(HttpStatus.NOT_FOUND);

      console.log('\n✅ 존재하지 않는 평가 초기화 방지 성공');
    });
