...
describe('GET /admin/performance-evaluation/final-evaluations/:id', () => {
  ...
  beforeEach(async () => {
    ...
    console.log('최종평가 조회 테스트 데이터 생성 완료:', {
  ...
  describe('최종평가 조회 성공 시나리오', () => {
    it('기본 최종평가를 조회할 수 있어야 한다', async () => {
      ...
      // When
      const response = await getFinalEvaluation(evaluationId).expect(200);
  ...
  describe('최종평가 조회 실패 시나리오', () => {
    it('잘못된 형식의 ID로 조회 시 400 에러가 발생해야 한다', async () => {
      // Given
      const invalidId = 'invalid-uuid';

      // When & Then
      await getFinalEvaluation(invalidId).expect(400);
    });
    ...
    it('존재하지 않는 ID로 조회 시 404 에러가 발생해야 한다', async () => {
      ...
      // When & Then
      await getFinalEvaluation(nonExistentId).expect(404);
    });
  });

  // ==================== 응답 구조 검증 ====================

  describe('최종평가 조회 응답 구조 검증', () => {
  ...
  describe('최종평가 조회 데이터 정합성', () => {
    ...
    it('확정 취소 후 조회 시 확정 정보가 null이어야 한다', async () => {
      // Given
      const employee = getRandomEmployee();
      const period = getRandomEvaluationPeriod();
      const confirmedBy = uuidv4();
