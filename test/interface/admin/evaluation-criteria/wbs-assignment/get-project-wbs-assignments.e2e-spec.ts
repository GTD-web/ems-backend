...
describe('GET /admin/evaluation-criteria/wbs-assignments/project/:projectId/period/:periodId', () => {
  ...
  beforeEach(async () => {
    ...
    console.log('프로젝트 WBS 할당 조회 테스트 데이터 생성 완료:', {
  ...
  describe('기본 조회', () => {
    it('특정 프로젝트의 특정 평가기간 WBS 할당을 조회할 수 있어야 한다', async () => {
      ...
      // When: 프로젝트의 WBS 할당 조회
      const response = await testSuite
        .request()
        .get(
          `/admin/evaluation-criteria/wbs-assignments/project/${project.id}/period/${periodId}`,
        )
        .expect(200);
  ...
  describe('다중 할당', () => {
    it('여러 직원에게 여러 WBS가 할당된 경우 모두 조회되어야 한다', async () => {
      // Given: 여러 직원에게 여러 WBS 할당
      const periodId = await createEvaluationPeriod();
      const project = getActiveProject();
  ...
  describe('필터링 검증', () => {
    it('다른 프로젝트의 WBS 할당은 조회되지 않아야 한다', async () => {
      // Given: 두 개의 프로젝트에 각각 WBS 할당
      const periodId = await createEvaluationPeriod();
      const employee = getRandomEmployee();
      const project1 = testData.projects.find((p) => p.isActive);
      const project2 = testData.projects.find(
        (p) => p.isActive && p.id !== project1?.id,
      );

      if (!project1 || !project2) {
        return; // 프로젝트가 부족한 경우 스킵
      }

      const project1Wbs = await getWbsItemsFromProject(project1.id);
      const project2Wbs = await getWbsItemsFromProject(project2.id);

      if (project1Wbs.length === 0 || project2Wbs.length === 0) {
        return; // WBS 항목이 부족한 경우 스킵
      }

      await createProjectAssignment(periodId, employee.id, project1.id);
      await createProjectAssignment(periodId, employee.id, project2.id);
      ...
      // Then: project1의 할당만 조회됨
      expect(response.body.wbsAssignments).toHaveLength(1);
      expect(response.body.wbsAssignments[0].projectId).toBe(project1.id);
    });
    ...
    it('다른 평가기간의 WBS 할당은 조회되지 않아야 한다', async () => {
      ...
      const response = await testSuite
        .request()
        .get(
          `/admin/evaluation-criteria/wbs-assignments/project/${project.id}/period/${period1Id}`,
        )
        .expect(200);
  ...
  describe('검증 실패 시나리오', () => {
    it('잘못된 UUID 형식의 projectId로 요청 시 400 에러가 발생해야 한다', async () => {
      ...
      await testSuite
        .request()
        .get(
          `/admin/evaluation-criteria/wbs-assignments/project/invalid-uuid/period/${periodId}`,
        )
        .expect(400);
    });
    ...
    it('잘못된 UUID 형식의 periodId로 요청 시 400 에러가 발생해야 한다', async () => {
      ...
      await testSuite
        .request()
        .get(
          `/admin/evaluation-criteria/wbs-assignments/project/${project.id}/period/invalid-uuid`,
        )
        .expect(400);
    });
    ...
    it('존재하지 않는 프로젝트 ID로 요청 시 빈 배열을 반환해야 한다', async () => {
      ...
      // When: 존재하지 않는 프로젝트로 조회
      const response = await testSuite
        .request()
        .get(
          `/admin/evaluation-criteria/wbs-assignments/project/${nonExistentProjectId}/period/${periodId}`,
        )
        .expect(200);
    ...
    it('존재하지 않는 평가기간 ID로 요청 시 빈 배열을 반환해야 한다', async () => {
      ...
      const response = await testSuite
        .request()
        .get(
          `/admin/evaluation-criteria/wbs-assignments/project/${project.id}/period/${nonExistentPeriodId}`,
        )
        .expect(200);
