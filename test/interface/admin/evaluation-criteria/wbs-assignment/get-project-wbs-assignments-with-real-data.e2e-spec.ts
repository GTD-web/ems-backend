/**
 * 프로젝트별 WBS 할당 조회 (GET) - 실제 데이터 기반 E2E 테스트
 *
...
describe('GET /admin/evaluation-criteria/wbs-assignments/project/:projectId/period/:periodId (실제 데이터)', () => {
  ...
  async function getProjectWithMultipleAssignments() {
    const result = await dataSource.query(`
      SELECT 
        wa."projectId" as project_id,
        wa."periodId" as period_id,
      ...
      FROM evaluation_wbs_assignment wa
      WHERE wa."deletedAt" IS NULL
      GROUP BY wa."projectId", wa."periodId"
      HAVING COUNT(*) >= 2
      LIMIT 1
    `);
    return result.length > 0 ? result[0] : null;
  ...
  describe('정상 조회', () => {
    it('특정 프로젝트의 특정 평가기간 WBS 할당을 조회할 수 있어야 한다', async () => {
      ...
      const response = await testSuite
        .request()
        .get(
          `/admin/evaluation-criteria/wbs-assignments/project/${data.project_id}/period/${data.period_id}`,
        )
    ...
    it('WBS 할당이 없는 경우 빈 배열을 반환해야 한다', async () => {
      ...
      const response = await testSuite
        .request()
        .get(
          `/admin/evaluation-criteria/wbs-assignments/project/${result[0].project_id}/period/${result[0].period_id}`,
        )
        .expect(HttpStatus.OK);
    ...
    it('여러 직원에게 여러 WBS가 할당된 경우 모두 조회되어야 한다', async () => {
      const data = await getProjectWithMultipleAssignments();
    ...
    it('다른 프로젝트의 WBS 할당은 조회되지 않아야 한다', async () => {
      ...
      // 모든 할당이 동일한 프로젝트인지 확인
      if (
        response.body.wbsAssignments &&
        response.body.wbsAssignments.length > 0
      ) {
        response.body.wbsAssignments.forEach((assignment: any) => {
          expect(assignment.projectId).toBe(data.project_id);
        });
      }
  ...
  describe('에러 케이스', () => {
    it('잘못된 UUID 형식의 projectId로 요청 시 400 에러가 발생해야 한다', async () => {
      ...
      const response = await testSuite
        .request()
        .get(
          `/admin/evaluation-criteria/wbs-assignments/project/invalid-uuid/period/${period.id}`,
        );
    ...
    it('존재하지 않는 프로젝트 ID로 요청 시 빈 배열을 반환해야 한다', async () => {
      ...
      const response = await testSuite
        .request()
        .get(
          `/admin/evaluation-criteria/wbs-assignments/project/${nonExistentId}/period/${period.id}`,
        )
        .expect(HttpStatus.OK);
    ...
    it('존재하지 않는 평가기간 ID로 요청 시 빈 배열을 반환해야 한다', async () => {
      ...
      const response = await testSuite
        .request()
        .get(
          `/admin/evaluation-criteria/wbs-assignments/project/${project.id}/period/${nonExistentId}`,
        )
        .expect(HttpStatus.OK);
