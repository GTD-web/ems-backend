...
### 1단계: 엔드포인트 분석
- 컨트롤러 파일 분석 (`*.controller.ts`)
- 엔드포인트 목록 정리 (HTTP 메서드, 경로, 요청/응답 구조)
- DTO 구조 파악
- 연계 관계 파악 (다른 컨트롤러와의 연계)
- 분석 결과를 `{feature}-endpoints.md` 파일로 문서화

### 2단계: 시나리오 문서 작성
- `test/usecase/scenarios/{feature}/README.md` 파일 생성
- 검증해야 할 시나리오를 엔드포인트 순서대로 정리
- 사용되는 컨트롤러 목록 명시
- 각 시나리오별 검증 사항 상세 기술
- 대시보드 상태 변경 검증 포함
...
- **파일 구조**:
  ...
  ├── base-{feature}.scenario.ts          # 기본 기능
  ├── {feature}-dashboard.scenario.ts     # 대시보드 검증 (필요시)
  ├── {feature}.scenario.ts               # 메인 시나리오 (Facade)
  └── index.ts                             # export 모음
  ```
- **작성 순서**:
  1. `base-{feature}.scenario.ts`: 기본 CRUD 기능 구현
  2. `{feature}-dashboard.scenario.ts`: 대시보드 상태 검증 기능 (필요시)
  3. `{feature}.scenario.ts`: 메인 시나리오 클래스 (Facade 패턴)
  4. `index.ts`: 모든 클래스 export

### 4단계: 테스트 코드 작성
- `{feature}.e2e-spec.ts` 파일 생성
- `beforeAll`: 테스트 환경 초기화 및 시나리오 인스턴스 생성
- `beforeEach`: 각 테스트마다 시드 데이터 생성 및 평가기간 설정
- `afterAll`: 테스트 환경 정리
- `describe` 블록으로 기능별 그룹화
- `it` 블록으로 개별 테스트 케이스 작성

### 작성 체크리스트
- [ ] 엔드포인트 분석 문서 작성 완료
- [ ] README.md에 시나리오 정리 완료
- [ ] Base 시나리오 클래스 작성 완료
- [ ] Dashboard 시나리오 클래스 작성 완료 (필요시)
- [ ] 메인 시나리오 클래스 작성 완료
- [ ] E2E 테스트 파일 작성 완료
- [ ] 모든 테스트 케이스가 HTTP 엔드포인트만 사용하는지 확인
