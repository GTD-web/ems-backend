...
│   └── self-evaluation.scenario.ts     # 자기평가 관리 시나리오
...
| 자기평가 관리 | ✅ | 자기평가 저장/제출/조회/초기화, 수정 가능 상태 관리 |
...
### 7. SelfEvaluationScenario
**기능**: WBS 자기평가 관리 (저장/제출/조회/초기화)  
**핵심 메서드**: `자기평가_전체_시나리오를_실행한다()`, `프로젝트별_자기평가_시나리오를_실행한다()`
...
- **자기평가 관리** (6개): 전체 시나리오, 프로젝트별 시나리오, 내용 초기화, 수정 가능 상태, 목록 조회, 전체 제출/초기화
...
### 자기평가 관리
- ✅ 자기평가 저장/제출/조회/초기화 (단일/전체/프로젝트별)
- ✅ 수정 가능 상태 관리 (자기평가/1차평가/2차평가/전체)
- ✅ Upsert 방식 저장 (기존 평가 수정, 신규 평가 생성)
- ✅ 낙관적 잠금을 통한 동시성 제어
