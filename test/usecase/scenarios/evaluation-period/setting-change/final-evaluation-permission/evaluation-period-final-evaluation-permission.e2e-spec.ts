 ...
 * - 평가기간 최종 평가 설정 수동허용 (PATCH /admin/evaluation-periods/{id}/settings/final-evaluation-permission)
 * - 활성 평가기간 조회 (GET /admin/evaluation-periods/active)
 * - 대시보드 직원 현황 조회 (GET /admin/dashboard/{evaluationPeriodId}/employees/status)
 */
describe('평가기간 최종 평가 설정 수동허용 E2E 테스트', () => {
  ...
  describe('평가기간 최종 평가 설정 수동허용', () => {
    it('최종 평가 설정 테스트용 평가기간을 생성한다', async () => {
      const today = new Date();
      const nextMonth = new Date(today);
      nextMonth.setMonth(today.getMonth() + 1);

      const createData = {
        name: '최종 평가 설정 테스트용 평가기간',
    ...
    it('평가기간 최종 평가 설정 수동허용을 변경한다', async () => {
      const updateData = {
      ...
      console.log('✅ 평가기간 최종 평가 설정 수동허용 변경 완료');
    });
