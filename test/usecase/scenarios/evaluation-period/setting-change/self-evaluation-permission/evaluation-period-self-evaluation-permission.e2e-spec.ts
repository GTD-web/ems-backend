...
describe('평가기간 자기 평가 설정 수동허용 E2E 테스트', () => {
  ...
  describe('평가기간 자기 평가 설정 수동허용', () => {
    it('자기 평가 설정 테스트용 평가기간을 생성한다', async () => {
      ...
      console.log(
        `✅ 자기 평가 설정 테스트용 평가기간 생성 완료: ${result.name} (${result.id})`,
      );
    });
    ...
    it('평가기간 자기 평가 설정 수동허용을 변경한다', async () => {
      ...
      const result = await apiClient.updateSelfEvaluationPermission(
        evaluationPeriodId,
        updateData,
      );
    ...
    it('대시보드에서 평가기간 직원 현황을 조회한다', async () => {
      ...
      // README.md 요구사항: evaluationPeriod.manualSettings.selfEvaluationSettingEnabled 확인 (true)
      ...
      console.log(
        `   - selfEvaluationSettingEnabled: ${firstEmployee.evaluationPeriod.manualSettings.selfEvaluationSettingEnabled} (기대값: true)`,
      );
      console.log(
