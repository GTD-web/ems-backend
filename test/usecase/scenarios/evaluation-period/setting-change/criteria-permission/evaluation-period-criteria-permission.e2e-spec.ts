...
describe('평가기간 기준 설정 수동허용 E2E 테스트', () => {
  ...
  describe('평가기간 기준 설정 수동허용', () => {
    ...
    it('대시보드에서 평가기간 직원 현황을 조회한다', async () => {
      ...
      expect(firstEmployee.evaluationPeriod.editableStatus.criteriaSettingEnabled).toBe(true);
      ...
      console.log(`✅ 대시보드 직원 현황 조회 완료: ${result.length}명, currentPhase: ${firstEmployee.evaluationPeriod.currentPhase}, criteriaSettingEnabled: ${firstEmployee.evaluationPeriod.criteriaSettingEnabled}, editableStatus: ${JSON.stringify(firstEmployee.evaluationPeriod.editableStatus)}`);
    });
  });
