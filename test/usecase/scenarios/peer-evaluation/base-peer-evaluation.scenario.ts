 ...
 * - 동료평가 취소
 */
export class BasePeerEvaluationScenario {
  protected apiClient: PeerEvaluationApiClient;
  protected evaluationQuestionScenario: EvaluationQuestionScenario;
  ...
  async 한명의_피평가자를_여러평가자에게_요청한다(config: {
    ...
    const result =
      await this.apiClient.requestPeerEvaluationToMultipleEvaluators(config);
  ...
  async 평가자의_동료평가목록을_조회한다(
    ...
    const result = await this.apiClient.getEvaluatorPeerEvaluations(
      evaluatorId,
      filter,
    );
  ...
  async 평가자에게_할당된_피평가자목록을_조회한다(
    ...
    const result = await this.apiClient.getEvaluatorAssignedEvaluatees(
      evaluatorId,
      query,
    );
  ...
  async 평가기간의_피평가자의_모든동료평가요청을_취소한다(
    ...
    const result = await this.apiClient.cancelPeerEvaluationsByPeriod(
      evaluateeId,
      periodId,
    );
