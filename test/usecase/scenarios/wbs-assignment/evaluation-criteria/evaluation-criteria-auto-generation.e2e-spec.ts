...
describe('평가기준 자동 생성 관리 시나리오', () => {
  ...
  describe('대시보드 API를 통한 평가항목 상태 검증', () => {
    it('직원 평가기간 현황 조회 시 평가항목 상태 정보가 포함되어야 한다', async () => {
      ...
      expect(직원현황.criteriaSetup?.evaluationCriteria.status).toBe('complete'); // evaluationCriteria.status가 'complete'인지 확인 (WBS 할당 완료)
      expect(직원현황.criteriaSetup?.evaluationCriteria.assignedProjectCount).toBeDefined(); // assignedProjectCount 확인
      expect(직원현황.criteriaSetup?.evaluationCriteria.assignedWbsCount).toBeDefined(); // assignedWbsCount 확인

      console.log('✅ 직원 평가기간 현황 조회 검증 완료');
      console.log(`  - 평가항목 상태: ${직원현황.criteriaSetup?.evaluationCriteria.status}`);
      console.log(
