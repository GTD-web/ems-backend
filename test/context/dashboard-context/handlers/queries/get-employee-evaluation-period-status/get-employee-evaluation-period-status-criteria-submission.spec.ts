 ...
 *
 * GetEmployeeEvaluationPeriodStatusHandler의 criteriaSetup.criteriaSubmission 필드가
 * 제대로 반환되는지 검증합니다.
 */
describe('GetEmployeeEvaluationPeriodStatusHandler - 평가기준 제출 상태 검증', () => {
  ...
  afterAll(async () => {
    const outputPath = path.join(
      __dirname,
      'get-employee-evaluation-period-status-criteria-submission-result.json',
    );
    const output = {
  ...
  it('평가기준 미제출 상태 - criteriaSubmission 필드가 올바르게 반환되어야 한다', async () => {
    ...
    expect(result?.criteriaSetup.criteriaSubmission).toBeDefined();
    expect(result?.criteriaSetup.criteriaSubmission.isSubmitted).toBe(false);
    expect(result?.criteriaSetup.criteriaSubmission.submittedAt).toBeNull();
    expect(result?.criteriaSetup.criteriaSubmission.submittedBy).toBeNull();

    testResults.push({
      testName:
        '평가기준 미제출 상태 - criteriaSubmission 필드가 올바르게 반환되어야 한다',
      result: {
        criteriaSetup: {
  ...
  it('평가기준 제출 상태 - criteriaSubmission 필드가 올바르게 반환되어야 한다', async () => {
    await 기본_테스트데이터를_생성한다();
    ...
    expect(result?.criteriaSetup.criteriaSubmission.submittedAt).toBeDefined();
    expect(result?.criteriaSetup.criteriaSubmission.submittedAt).not.toBeNull();
    expect(result?.criteriaSetup.criteriaSubmission.submittedBy).toBe(
      submittedBy,
    );

    testResults.push({
      testName:
        '평가기준 제출 상태 - criteriaSubmission 필드가 올바르게 반환되어야 한다',
      result: {
        criteriaSetup: {
          criteriaSubmission: result?.criteriaSetup.criteriaSubmission,
        },
  ...
  it('평가기준 제출 후 초기화 - criteriaSubmission 필드가 올바르게 반환되어야 한다', async () => {
    await 기본_테스트데이터를_생성한다();
    ...
    testResults.push({
      testName:
        '평가기준 제출 후 초기화 - criteriaSubmission 필드가 올바르게 반환되어야 한다',
      result: {
        criteriaSetup: {
          criteriaSubmission: result?.criteriaSetup.criteriaSubmission,
        },
