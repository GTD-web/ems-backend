...
import { GetEmployeeEvaluationPeriodStatusQuery } from '@context/dashboard-context/handlers/queries/get-employee-evaluation-period-status/get-employee-evaluation-period-status.handler';
...
describe('DashboardService.getEmployeeCompleteStatus', () => {
  ...
  describe('getEmployeeCompleteStatus', () => {
    it('정상적으로 직원의 평가 현황 및 할당 데이터를 조회할 수 있어야 한다', async () => {
      ...
      // When - 핸들러를 직접 사용하여 조회
      const statusHandler = module.get<GetEmployeeEvaluationPeriodStatusHandler>(
        GetEmployeeEvaluationPeriodStatusHandler,
      );
      const assignedHandler = module.get<GetEmployeeAssignedDataHandler>(
        GetEmployeeAssignedDataHandler,
      );

      const statusQuery = new GetEmployeeEvaluationPeriodStatusQuery(
        evaluationPeriodId,
        employeeId,
      );
      const assignedQuery = new GetEmployeeAssignedDataQuery(
        evaluationPeriodId,
        employeeId,
      );

      const statusData = await statusHandler.execute(statusQuery);
      const assignedData = await assignedHandler.execute(assignedQuery);
    ...
    it('모든 필드의 타입이 올바른지 확인한다', async () => {
      ...
      // criteriaSettingEnabled는 선택적 필드일 수 있음
      if (statusData!.evaluationPeriod.criteriaSettingEnabled !== undefined) {
    ...
    it('존재하지 않는 평가기간으로 조회 시 null 또는 예외가 발생해야 한다', async () => {
      // Given
      await 테스트데이터를_생성한다();
      ...
      // Then - statusHandler는 null을 반환할 수 있음
      const statusData = await statusHandler.execute(statusQuery);
      expect(statusData).toBeNull();

      // Then - assignedHandler는 NotFoundException을 던짐
      await expect(assignedHandler.execute(assignedQuery)).rejects.toThrow(
        '평가기간을 찾을 수 없습니다',
      );
    });

    it('존재하지 않는 직원으로 조회 시 null 또는 예외가 발생해야 한다', async () => {
      // Given
      await 테스트데이터를_생성한다();
