...
let WbsAssignmentBusinessService = WbsAssignmentBusinessService_1 = class WbsAssignmentBusinessService {
    ...
    constructor(evaluationCriteriaManagementService, commandBus, employeeService, projectService, evaluationLineService, evaluationLineMappingService, evaluationWbsAssignmentService) {
    ...
    async WBS를_할당한다(params) {
        ...
        try {
            await this.commandBus.execute(new handlers_1.평가활동내역을생성한다(params.periodId, params.employeeId, 'wbs_assignment', 'created', 'WBS 할당', undefined, 'wbs_assignment', assignment.id, params.assignedBy, undefined, {
                wbsItemId: params.wbsItemId,
                projectId: params.projectId,
            }));
        }
    ...
    async WBS_할당을_취소한다(params) {
        ...
        try {
            await this.commandBus.execute(new handlers_1.평가활동내역을생성한다(periodId, employeeId, 'wbs_assignment', 'cancelled', 'WBS 할당 취소', undefined, 'wbs_assignment', params.assignmentId, params.cancelledBy, undefined, {
                wbsItemId,
    ...
    async WBS를_대량으로_할당한다(params) {
        ...
        await Promise.all(assignments.map(async (assignment) => {
            try {
                await this.commandBus.execute(new handlers_1.평가활동내역을생성한다(assignment.periodId, assignment.employeeId, 'wbs_assignment', 'created', 'WBS 할당', undefined, 'wbs_assignment', assignment.id, params.assignedBy, undefined, {
                    wbsItemId: assignment.wbsItemId,
                    projectId: assignment.projectId,
                }));
            }
    ...
    async 평가라인을_자동으로_구성한다(employeeId, wbsItemId, projectId, periodId, createdBy) {
        ...
        console.log(...oo_oo(`646790541_916_4_921_6_4`, '🔍 직원 정보:', {
            id: employee.id,
            name: employee.name,
        ...
        console.log(...oo_oo(`646790541_930_4_934_6_4`, '🔍 프로젝트 정보:', {
            id: project.id,
            name: project.name,
