...
| `with_evaluations` | + **성과평가입력 완료**                   | 2단계 완료 | 🔄 구현예정 |
| `full`             | + **성과평가 완료** (최종평가까지 완료)   | 3단계 완료 | ✅ 구현완료 |

> ⚠️ **참고**: `with_evaluations` 시나리오는 아직 구현되지 않았습니다. 현재는 `full` 시나리오에서 `stateDistribution`을 조정하여 유사한 효과를 낼 수 있습니다.
...
**5. WITH_EVALUATIONS - 2단계: 성과평가입력 완료** 🔄 (구현예정)

자기평가, 하향평가, 동료평가가 **모두 입력 완료**된 상태

> ⚠️ 현재 구현되지 않았습니다. 대신 `full` 시나리오에서 `stateDistribution`을 조정하세요:
